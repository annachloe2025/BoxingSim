...
## 2026-05-30
- **「読み合い(que)」の設計フェーズに入った**（opus 4.8 新セッション。壁に直面しより広く深い設計を要望）。レポート: `reports/2026-05-30-que-ai-proposals.html`。
  - **壁の正体**: 決定核が「今の状態→1手」の反応型（`scoreActions` も新 `Playbook` も同型）。良情報の反応型どうしは均衡（お見合い・鏡写し・時間切れ）に落ちる。段階1〜5 を積んでも動かなかったのは積む場所＝反応型の決定核が壁だから。
  - **目標アーキ（2層）**: 技FSMの上に「反応層（既定）＋型(que)層（戦略）」。型＝技/移動＋観る点＋分岐＋撤退を1セットにした"目と判断を持つミニ戦術"。例＝誘って空振り→反撃（格ゲーの shimmy）。
  - **Web調査で裏取り**: shimmy/差し合い、Behavior Tree（reactive sequence＋条件付き abort）、相手の癖テーブル（Killer Instinct "Shadow"/FightingICE）。回避＝MCTS（中身が見えず理念違反）。北極星＝三すくみ＋Yomi。
  - **知覚遅延の再評価**: presence/detail の2段遅延（段階3で実装済）は厄介者でなく「先読み(que)を必要にする土台」。遅延より速い技は反応不能→事前に読むしかない（Firoiu "At Human Speed"・FightingICE）。知覚層⟂判断層を分ければ型は複雑化しない。
  - **3案**: 案1 最小BT型／案2 ＋相手の癖テーブル／案3 目的逆算プランナ。推奨＝案1→案2→案3。ユーザーは案1から着手で合意。
- **公開・音声化パイプライン整備**（ユーザー指摘で発覚）。`tools/build-index.js`（HTML自動インデックス）は 2026-05-27 に手動ダッシュボード README.html へ移行のため意図的に廃止済みと確認。push は Stop フックの `publish-audio.js`（`git add -A`）が全ファイルを公開。パイプライン仕様（`docs/superpowers/specs/2026-05-28-report-audio-pipeline-design.md`）を CLAUDE.md に明記。README ダッシュボードを手動更新（新レポート追記・「現在地」更新・全レポートのリンク健全性確認・ローカルで一時欠けていた4レポートを git から復元）。
- **リファクタ Step 0 完了（挙動不変・Identity PASS）**（詳細: `reports/2026-05-30-refactor-step0.html`）。implementer 実施・各ステップで verify.js MD5 確認。**MD5 `46967075cd72e1949417dc504fcb0a13` が編集前後で完全一致**（独立再検証済み・変更は src/index.html のみ）。
  - 死にコード4削除: `DECIDE_EVERY`/`allMoveKeys`/`moveData`/`situation`（呼び出しゼロを grep 確認）。
  - セクションバナー (A)〜(G) 付与（宣言の物理移動は MD5 リスクのため見送り、現位置に目印。ファイルは依然レイヤー交錯）。
  - 判断層に「反応層／型(que)層」ラベルをコメントで明示。
  - 次（Step 1）: 案1 実装（型層が"効く時だけ"前に出て、それ以外は反応層に戻す＝休眠中の反応層を復活）。意図的な挙動変更なので新 baseline。予測モデルは「コードは残すが案1では OFF」。
