...
- `audio/scripts/YYYY-MM-DD-トピック.txt` の新規作成 — レポートと**同じファイル名**で、耳で聞いてわかる読み上げ原稿（後述「読み上げ原稿」）。レポートHTMLを作るときは必ずセットで作る。
...
レポートを作ったら、同じファイル名で `audio/scripts/` に読み上げ原稿（.txt）を書く。
これは後で edge-tts（Nanami 音声）で MP3 になり、ユーザーがスマホで聞く。**耳で聞いて理解できる話し言葉**にすること。

- 記号・コード・URL・ファイルパスを読み上げ文に入れない。数値は言葉にする（例: 「28%」→「およそ二十八パーセント」）。
- 見出し記号（#, *, - など）を使わない。段落を改行で区切り、自然に話す。
- 内容は「何を試して」「何がわかったか」「次どうするか」を**しっかり詳しく**。要約しすぎない。
- 宣伝調にしない。プロジェクト理念どおり、観察された事実とその構造的解釈を語る。
- 例は `audio/scripts/2026-05-28-prediction-strength-ko-artifact.txt` を参照。

（音声化・公開は `tools/publish-audio.js` が自動でやる。reporter は原稿を書くだけでよい。）
